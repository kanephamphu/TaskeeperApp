      ...
      var task_title = this.props.item.task_title;
        ...
        return(
            <View style={styles.image_container}>
              <TouchableOpacity onPress={()=>this.props.stackScreen(this.props.item._id)} style={{justifyContent:'center',marginLeft:10,height:55,width:55,backgroundColor:'white', shadowOffset: { width: 0, height: 3 },
    shadowColor: 'green',
    shadowOpacity: 0.5,
    elevation: 10,
    borderColor: '#71B7B7',
                      ...
                      <View style={{width:width-160}}>
                        <TouchableOpacity onPress={()=>this.props.stackScreen(this.props.item._id)}>
                          <View style={{flexDirection:'row'}}>
                            ...
                            <Text style={{fontStyle:'italic'}}>{new Date(this.props.item.created_time).toLocaleTimeString()}  </Text>
                        ...
                        {new Date(this.props.item.created_time).toLocaleDateString()==d.toLocaleDateString()?
                          <>
                            <Text style={{fontWeight:'bold',fontSize:12,color:'red',top:0,right:0,position:'absolute'}}>(new)</Text>
                          </>
                        :null}
                      </View>
                    </View>
